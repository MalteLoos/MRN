...
class SlowVisualEncoder(nn.Module):
    ...
    def compute_flow(
        ...
        """
        Run frozen RAFT-Small to produce a dense optical-flow map.

        The returned flow tensor is **detached** (RAFT is frozen).
    ...
    def encode_flow(self, flow: torch.Tensor) -> torch.Tensor:
        """
        Run the **trainable** FlowEncoder CNN on a pre-computed flow map.

        This is the only part of the slow path that carries gradients.
        During PPO updates, call this on stored flow maps so the
        FlowEncoder receives gradient updates.
    ...
    def forward(
        ...
        Convenience method that chains ``compute_flow`` → ``encode_flow``.
...
class DronePolicy(nn.Module):
    ...
    **Fast path** — updated on every IMU tick:
    ...
    def __init__(self, cfg: ModelConfig | None = None):
        ...
        # Cached RAFT flow map — stored in rollout buffer for PPO re-encoding
    ...
    def update_vision(
        ...
        Returns:
            (flow, vis_feat):
                flow:     (B, 2, H, W) — RAFT output (detached).
                vis_feat: (B, flow_feature_dim) — FlowEncoder output
                          (detached for cache; live gradients flow through
                          ``encode_flow`` during PPO updates).
        """
        flow, vis_feat = self.slow_visual(img_prev, img_curr)
        self._flow_cache = flow  # already detached (RAFT is frozen)
        self._vis_feat_cache = vis_feat.detach()  # cache for fast-path inference
        return flow, vis_feat
    ...
    def _get_vis_feat(
        ...
        """
        Return visual features, re-encoding from stored flow maps when
        available so the FlowEncoder receives gradients during PPO updates.

        Priority:
        1. ``obs["flow"]`` — re-run FlowEncoder (trainable, with gradients).
        2. ``obs["vis_feat"]`` — pre-computed feature (no FlowEncoder grad).
        3. Internal ``_vis_feat_cache`` — inference fallback.
        4. Zeros — before first camera frame.
        """
        if "flow" in obs:
            # Re-run the trainable FlowEncoder → gradients flow through it
            return self.slow_visual.encode_flow(obs["flow"])
        if "vis_feat" in obs:
            return obs["vis_feat"]
    ...
    def evaluate_actions(
        ...
        ``obs["flow"]`` **should** be provided so the trainable FlowEncoder
        is re-run with live gradients.  Falls back to ``obs["vis_feat"]``
        if flow maps are not available (but FlowEncoder won't be trained).
...
class DroneAgent:
    ...
    @torch.no_grad()
    def update_vision(self, obs: dict) -> Tuple[torch.Tensor, torch.Tensor] | None:
        """
        ...
        Returns:
            (flow, vis_feat):
                flow:     (1, 2, H, W) — RAFT optical-flow map (detached).
                vis_feat: (1, flow_feature_dim) — FlowEncoder output (detached).
            ``None`` if skipped.
        """
        ...
        flow, vis_feat = self.policy.update_vision(self._prev_frame, cam_curr)
...
class RolloutBuffer:
    ...
    The RAFT optical-flow map is computed at the slower camera rate and
    stored alongside each transition (it stays constant between camera
    frames).  During PPO updates the ``FlowEncoder`` is **re-run** on the
    stored flow maps so that it receives gradient updates — we never need
    to re-run RAFT itself, keeping the update cheap while still training
    the visual encoder.
    ...
    def __init__(
        ...
        Args:
            ...
            flow_hw:      (H, W) spatial size of the RAFT flow maps.
            device:       "cpu" or "cuda".
        ...
        # Store RAFT flow maps so FlowEncoder can be re-run with gradients
        flow_h, flow_w = flow_hw
        self.flow = torch.zeros(
            buffer_size, 2, flow_h, flow_w, device=self.device
        )
    ...
    def sample_batches(
        ...
        ``obs["flow"]`` is provided so  ``evaluate_actions`` can re-run the
        trainable FlowEncoder with live gradients.
        """
        n = self.ptr
...
class PPOTrainer:
    ...
    During the PPO update, the stored RAFT flow maps are passed back
    through the **trainable** ``FlowEncoder`` with live gradients, so
    the visual encoder is properly trained alongside the fast-path MLP
    and actor/critic heads.  Only the ``FlowEncoder`` is re-run — frozen
    RAFT itself is never re-executed.
    ...
    def __init__(
        ...
        self.buffer = RolloutBuffer(
            ...
            flow_hw=(self.cfg.cam_height, self.cfg.cam_width),
            device=device,
    ...
    def collect_rollout(self, env) -> dict:
        ...
        for _ in range(self.rollout_steps):
            ...
            # Get cached flow map for storage (used to re-run FlowEncoder
            # with gradients during the PPO update)
            flow = self.agent.policy._flow_cache
            if flow is None:
                flow = torch.zeros(
                    1, 2, self.cfg.cam_height, self.cfg.cam_width,
                    device=self.agent.device,
                )

            # For inference, use the detached vis_feat cache (no grad needed)
